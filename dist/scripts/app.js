...
function youtubeVideo(elId, videoId) {
	var playerVars = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
...
var _cardSlider = __webpack_require__(127);

var _cardSlider2 = _interopRequireDefault(_cardSlider);

var _fullpage = __webpack_require__(128);

var _fullpage2 = _interopRequireDefault(_fullpage);

var _sec = __webpack_require__(129);

var _sec2 = _interopRequireDefault(_sec);

var _sec3 = __webpack_require__(130);

var _sec4 = _interopRequireDefault(_sec3);

var _sec5 = __webpack_require__(131);
...
var _mobileNav = __webpack_require__(133);

var _mobileNav2 = _interopRequireDefault(_mobileNav);

var _textGallery = __webpack_require__(135);

var _textGallery2 = _interopRequireDefault(_textGallery);

var _apps = __webpack_require__(136);
...
var app = {
	...
	bindEvents: function bindEvents() {
		...
		(0, _textGallery2.default)();
		(0, _mobileNav2.default)();
		(0, _fullpage2.default)();
		(0, _sec4.default)();
		(0, _sec6.default)({ el: '.sec2__bottom' });
		(0, _modal2.default)();
		(0, _apps2.default)();
		(0, _sec2.default)('game');
		(0, _cardSlider2.default)({ el: '.sec5__slider' });
	}
};
...
function cardSlider() {
	var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { el: el };

	var slider = $(options.el);
	var slidesContainer = slider.find('.sec5__slider-slides');
	var nav = $('.sec5__tags');
	var navButtons = $('.sec5__tags-btn');

	var animationInProgress = false;
	var activeItem = '';
	var activeItemName = navButtons.first().data('name');

	navButtons.on('click', function (e) {
		e.preventDefault();

		var self = $(this);
		var btnName = self.attr('data-name');
	...
	function setActiveSlide(name) {
		...
		var item = slidesContainer.find('.sec5__slider-card:last-child');
		var nextItem = slidesContainer.find('.sec5__slider-card[data-name="' + name + '"]');
...
exports.default = fullpageContent;
// import $ from "jquery";
...
function fullpageContent() {
	...
	function createFullpage() {
		runPlugin = true;
		var anchors = [];
		var sectionsLength = document.querySelectorAll('.section').length;
	...
	if (fullpageEl) {
		var media = window.matchMedia('(max-width: 1700px)').matches;
...
exports.default = sec4Slider;

function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } }

function sec4Slider(startCategory) {
	var slider = document.querySelector('.sec4__slider');
	var activeIndex = 0;
	var activeCategory = startCategory;
	var animationDuration = '0.5s';
	var init = true;
	var disableLinks = false;

	if (slider) {
		var setActiveItem = function setActiveItem(category) {
			activeCategory = category;
			var nowActiveItem = body.querySelector('.sec4__slider-cards_active');
			var nowActiveItemHeight = void 0;

			var newActiveItem = body.querySelector('[data-category="' + category + '"]');
			var newActiveItemHeight = newActiveItem.clientHeight;

			var nowActiveNavLink = nav.querySelector('.state_active');
			var newActiveNavLink = nav.querySelector('[data-category="' + category + '"]');
		...
		var cards = slider.querySelectorAll('.sec4__slider-cards');
		var nav = slider.querySelector('.sec4__slider-nav');
		var links = slider.querySelectorAll('.sec4__slider-nav-link');
		var body = slider.querySelector('.sec4__slider-body');
...
var _youtubeVideo = __webpack_require__(90);
...
function sec1() {
	(0, _youtubeVideo2.default)('sec1Video', 'i-kVeQOmMSo', {
		controls: 0,
		autoplay: 1
	}, {}, {
...
function sec2() {
	...
	var slider = $(options.el);
	var slidesContainer = slider.find('.sec2__bottom-slider');
	var nav = $('.sec2__slider-nav');
	var navItems = $('.sec2__slider-nav-item');
	var navButtons = $('.sec2__slider-nav-link');
	...
	function setActiveSlide(name) {
		animationInProgress = true;
		var item = slidesContainer.find('.sec2__slider-item:last-child');
		var nextItem = slidesContainer.find('.sec2__slider-item[data-name="' + name + '"]');
...
function modal() {
	...
	var closeLinks = document.querySelectorAll('[data-close-modal]');

	[].concat(_toConsumableArray(modals)).forEach(function (modal) {
	...
	[].concat(_toConsumableArray(closeLinks)).forEach(function (closeLink) {
...
var _anchorScroll = __webpack_require__(134);
...
function mobileNav() {
	...
	var openNavLinks = document.querySelectorAll('[data-open-nav]');
	var closeNavLinks = document.querySelectorAll('[data-close-nav]');
	var open = false;
	...
	if (nav) {
		...
		(0, _anchorScroll2.default)(function () {
			if (document.documentElement.classList.contains('disable-scroll')) {
				closeNav();
			}
		});

		[].concat(_toConsumableArray(openNavLinks)).forEach(function (link) {
		...
		[].concat(_toConsumableArray(closeNavLinks)).forEach(function (link) {
	...
	function openNav() {
		...
		if (nav) {
			var navOpened = function navOpened() {
...
function anchorScroll(callback) {
...
function textGallery() {
	var galleries = document.querySelectorAll('.tg');

	if (galleries.length > 0) {
		[].concat(_toConsumableArray(galleries)).forEach(function (gallery) {
			var links = gallery.querySelectorAll('.tg__list-link');
			var items = gallery.querySelectorAll('.tg__list-item');
			var description = gallery.querySelectorAll('.tg__description');
			var videos = gallery.querySelector('.tg__videos');
			...
			function setActiveItem(link) {
				...
				activeIndex = +index;
				var videoId = link.getAttribute('data-video-link');
				var activeItem = gallery.querySelector('.tg__list-item_active');
				var activeDescription = gallery.querySelector('.tg__description_active');

				var newActiveItem = items[index];
				var newactiveDescription = description[index];
				...
				if (activeDescription) {
					...
					$(activeDescription).fadeOut(400, function () {
			...
			function createVideoFrame(videoId) {
				player = (0, _youtubeVideo2.default)(videos, videoId, {
					controls: 0,
					autoplay: 1
				}, {
					onReady: function onReady(e) {
...
function apps() {
	...
	var headWrap = document.querySelector('.apps__head-wrap');
	var bagesList = document.querySelector('.apps__top-bages_top');
	var appsBottom = document.querySelector('.apps__bottom');
	var windowHeight = document.documentElement.clientHeight;

	var headWrapBottom = void 0;
	...
	function createParallax() {
		...
		var phoneTween = new TimelineMax({ paused: true });
		phoneTween.add(TweenMax.to('.apps__phone-wrap', 1, { css: { yPercent: 20 }, ease: Linear.easeNone }));

		var headTween = new TimelineMax({ paused: true });
		headTween.add(TweenMax.to('.apps__head', 1, { css: { yPercent: -40 }, ease: Linear.easeNone }));

		var sceneHead = new ScrollMagic.Scene().setPin('.apps__top').addTo(controller);
		// .addIndicators()

		var scenePhone = new ScrollMagic.Scene({ duration: 800 }).addTo(controller);
		// .addIndicators()

		var sceneTop = new ScrollMagic.Scene({ offset: topOffset }).addTo(controller)
		// .addIndicators()
		.triggerHook("onLeave");

		var sceneBottom = new ScrollMagic.Scene({ triggerElement: '.apps__bottom', offset: 200, duration: 500 }).addTo(controller);
		// .addIndicators()
	...
	if (document.querySelector('.apps')) {
		var media = window.matchMedia('(max-width: 1024px)').matches;
